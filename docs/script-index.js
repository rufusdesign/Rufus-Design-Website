...
// GRID ITEM STAGGER & DELAY //
setTimeout(() => {
  const itemsPerRow = 10;
  document.querySelectorAll('.grid-item').forEach((thumb, i) => {
    const indexInRow = i % itemsPerRow;
    thumb.style.animationDelay = `${indexInRow * 0.03}s`; // ver small stagger
    thumb.classList.add('is-visible'); // triggers the animation
  });
}, 5250); // delay before the whole stagger starts
...
class PageTransition {
  ...
  showExitOverlay(callback) {
    ...
    // Animate page content: shrink + slide
    ...
    // Wait for overlay transition to complete before navigating
    const onEnd = () => {
